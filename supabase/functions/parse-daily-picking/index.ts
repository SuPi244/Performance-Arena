...
function parse(text:string){
 ...
 let period=date?`${date[1]}-${date[2]}-${date[3]}`:null;
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(mode==="preview")return J({
   ok:true,preview:true,parser_stage:"daily_picking_parsed_v5",
  ...
  await db.from("imports").update({status:"imported",period_start:p.period,period_end:p.period,parser_version:"daily-picking-v7"}).eq("id",import_id);
  return J({ok:true,preview:false,parser_stage:"daily_picking_committed_v5",attempted_count:obs.length,inserted_count:w?.length??0,merge_guard:"canonical-v153-identity"});
