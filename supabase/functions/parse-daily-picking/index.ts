...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(mode==="preview")return J({
   ok:true,preview:true,parser_stage:"daily_picking_parsed_v4",
  ...
  await db.from("imports").update({status:"imported",period_start:p.period,period_end:p.period,parser_version:"daily-picking-v6"}).eq("id",import_id);
  return J({ok:true,preview:false,parser_stage:"daily_picking_committed_v4",attempted_count:obs.length,inserted_count:w?.length??0,merge_guard:"canonical-v152"});
